...
<template>
  <div class="settings-tab">
    ...
    <div class="card">
      <h3>🪜 Ladder</h3>
      <div class="setting-row">
        <label>Spacing multiplier: <strong>{{ ladderSpacing.toFixed(1) }}x</strong></label>
    ...
    <div class="card">
      ...
      <div class="toggle-row">
        <button :class="['btn', hwmEnabled ? 'active' : '']" @click="hwmEnabled = !hwmEnabled">
          HWM: {{ hwmEnabled ? 'ON' : 'OFF' }}
        </button>
        <button :class="['btn', pnlTrailEnabled ? 'active' : '']" @click="pnlTrailEnabled = !pnlTrailEnabled">
          PnL: {{ pnlTrailEnabled ? 'ON' : 'OFF' }}
        </button>
      </div>
      <div class="metrics">
        <div>
    ...
    <button class="btn save-btn" @click="saveSettings" :disabled="saving">
      💾 Save Settings
    </button>
...
.message { text-align: center; margin-top: 10px; color: #00ff88; }
</style>