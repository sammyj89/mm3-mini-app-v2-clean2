...
function showPopup(params) {
  return new Promise(resolve => {
    // Use Telegram's native popup if available, else simple confirm
...
async function selectSlot(slot) {
  ...
  try {
    if (slot === 'new') {
      ...
      await apiPost('/api/rotate_symbol', { old: slot, new: chosenSymbol.value })
    }
    emit('slotChanged')
...
<template>
  <div>
    <button class="btn scanner-btn" @click="startScan">📡 Run Scanner</button>
